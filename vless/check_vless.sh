...
save_config() {
    local port=$1
    if [[ ! -f ~/domains/11.spfnas.us.kg/serv00-script/vless/config.json ]]; then
        uuid=$(generate_uuid)
        cat <<EOL > ~/domains/11.spfnas.us.kg/serv00-script/vless/config.json
{
    "uuid": "$uuid",
    "port": $port
}
EOL
        ...
        jq --arg port "$port" '.port = ($port | tonumber)' ~/domains/11.spfnas.us.kg/serv00-script/vless/config.json > ~/domains/11.spfnas.us.kg/serv00-script/vless/config_tmp.json && mv ~/domains/11.spfnas.us.kg/serv00-script/vless/config_tmp.json ~/domains/11.spfnas.us.kg/serv00-script/vless/config.json
...
deploy_vless() {
    ...
    npm install
    # 启动vless项目
    ~/.npm-global/bin/pm2 start ~/domains/11.spfnas.us.kg/serv00-script/vless/app.js --name vless
    # 保存pm2进程状态
...
main() {
    ...
    if [ "$port_provided" = true ]; then
        ...
        if [[ -f config.json ]]; then
            ...
            echo -e "VLESS节点信息: vless://${uuid}@11.spfnas.us.kg:${port}?flow=&security=none&encryption=none&type=ws&host=11.spfnas.us.kg&path=/&sni=&fp=&pbk=&sid=#$USER.serv00.vless"
